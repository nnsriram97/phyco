...
def init() -> int | None:
    ...
    try:
        ...
        # Add error handling for CPU affinity setting
        try:
            os.sched_setaffinity(0, cpu_affinity)
        except (OSError, ValueError) as e:
            log.warning(f"Failed to set CPU affinity: {e}. Continuing without CPU affinity optimization.")
    except pynvml.NVMLError as e:
        log.warning(f"Failed to set device affinity: {e}")
    except Exception as e:
        log.warning(f"Unexpected error during device affinity setup: {e}")
    ...
    try:
        ...
        # Set device limit on the current device. Some cloud/MIG partitions reject 128, so ignore failures.
        limit_id = ctypes.c_int(0x05)
        desired_value = ctypes.c_int(128)
        p_value = ctypes.cast((ctypes.c_int * 1)(), ctypes.POINTER(ctypes.c_int))
        result = _libcudart.cudaDeviceSetLimit(limit_id, desired_value)
        if result != 0:
            err_msg = "unknown error"
            try:
                _libcudart.cudaGetErrorString.restype = ctypes.c_char_p
                err_msg = _libcudart.cudaGetErrorString(result).decode("utf-8")
            except AttributeError:
                err_msg = f"code {result}"
            log.warning(
                f"cudaDeviceSetLimit cudaLimitMaxL2FetchGranularity={desired_value.value} failed on this device ({err_msg}). "
                "Continuing with the default limit."
            )
        else:
            _libcudart.cudaDeviceGetLimit(p_value, limit_id)
    except OSError as e:
        log.warning(f"Failed to load libcudart for cudaDeviceSetLimit: {e}. Continuing with default limit.")
