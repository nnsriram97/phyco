...
show_help() {
    ...
    echo "  --run_ids IDS             Run IDs to process (e.g., 0,1,2 or 0-3 or 0,2,5-7)"
    ...
    echo "  $0 --batch_input_json /path/to/config.json --run_ids 0,2,5-7"
}
...
# Set CUDA_VISIBLE_DEVICES based on the number of GPUs requested
if [ "$NUM_GPUS" -gt 1 ]; then
    # For multi-GPU, create a comma-separated list of GPU IDs (0,1,2,...)
    GPU_LIST=""
    ...
    echo "Setting CUDA_VISIBLE_DEVICES=$CUDA_VISIBLE_DEVICES for $NUM_GPUS GPUs"
else
    # For single GPU, default to GPU 0
...
CMD="torchrun --nproc_per_node=${NUM_GPUS} --master_port ${MASTER_PORT} examples/video2world.py \
