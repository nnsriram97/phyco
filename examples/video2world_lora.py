...
def setup_lora_pipeline(config, dit_path, text_encoder_path, args):
    ...
    if text_encoder_path:
        ...
        pipe.text_encoder = CosmosT5TextEncoder(device="cuda", cache_dir=text_encoder_path, local_files_only=True, use_8bit=True)
    ...
    if False: # TODO: add prompt refiner back in
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Prepend a string to the output filename",
    )
...
def process_single_generation(
    pipe, input_path, prompt, output_dir, output_filename, negative_prompt, num_conditional_frames, guidance, seed, benchmark=False
):
    # Validate input file
    ...
    if video is not None:
        ...
        if not output_filename:
            output_filename = input_path.split("/")[-1].split(".")[0] + "_v2w.mp4"
    ...
    if args.batch_input_json:
        ...
        for idx, item in enumerate(batch_inputs):
            ...
            process_single_generation(
                ...
                output_filename=args.prepend_fname + output_video,
