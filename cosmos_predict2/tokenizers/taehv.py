            ...
            if i == len(model):
                ...
                if isinstance(b, MemBlock):
                    ...
                    if mem[i] is None:
                        ...
                        xt_new = b(xt, mem[i])
                        # avoid in-place copy that can break autograd versioning
                        mem[i] = xt
...
class TAEHV(nn.Module):
    def __init__(self, checkpoint_path="taehv.pth", decoder_time_upscale=(True, True), decoder_space_upscale=(True, True, True), patch_size=1, latent_channels=16):
        ...
        self.encoder = nn.Sequential(
            conv(self.image_channels*self.patch_size**2, 64), nn.ReLU(inplace=False),
        ...
        self.decoder = nn.Sequential(
            ...
            nn.ReLU(inplace=False), conv(n_f[3], self.image_channels*self.patch_size**2),
        )
