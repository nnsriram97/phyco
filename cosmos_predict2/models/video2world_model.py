...
class Predict2Video2WorldModelConfig:
    ...
    # This was previously injected dynamically but doing so violates OmegaConf's
    # structured config constraints. By defining it explicitly we allow Hydra
    # overrides such as `+ema=power` while keeping the config struct‐safe.
...
class Predict2Video2WorldModel(ImaginaireModel):
    ...
    def validation_step(
        ...
        """Validation step that takes only the first frame as input to the pipeline and
        saves the generated video to disk.

        Args:
            data_batch (dict[str, torch.Tensor]): A batch coming from the validation
                dataloader. It is expected to contain at least the following keys:
                - ``video`` (or the key defined by ``self.input_data_key``): ``uint8``
                  tensor shaped ``(B, C, T, H, W)``.
                - ``physprop``: ``float32`` tensor shaped ``(B, C_phys, H, W)``.
                - ``fps`` (optional): Frames‐per‐second as ``int`` or 0-D tensor.
            iteration (int): The current global training iteration. Used only for
                naming the output files.

        Returns:
            output_batch (dict[str, torch.Tensor]): Dictionary with the generated
                videos (on CPU) under key ``generated_videos``.
            loss (torch.Tensor): Dummy loss tensor (always ``0``) to satisfy the
                trainer interface.
        """
        self.eval()
        ...
        # Fetch inputs.
        # ------------------------------------------------------------------
        video_key = self.input_data_key if self.input_data_key in data_batch else "video"
        video: torch.Tensor = data_batch[video_key]  # (B, C, T, H, W) uint8
        fps_in: torch.Tensor | int | None = data_batch.get("fps", None)
        ...
        # Use the same job directory where model checkpoints are saved
        # save_dir = os.path.join("checkpoints", self.config.job.project, self.config.job.group, self.config.job.name, "val_outputs")
        save_dir = os.path.join(os.getcwd(), "val_outputs")
        os.makedirs(save_dir, exist_ok=True)

        for b in range(batch_size):
            # ------------------------------------------------------------------
            # Prepare the first frame & physprop for the pipeline.
            # ------------------------------------------------------------------
            first_frame_tensor = video[b, :, 0]  # (C, H, W)
            # Convert to H, W, C and NumPy uint8.
            first_frame_np = first_frame_tensor.permute(1, 2, 0).cpu().numpy()
            
            # ------------------------------------------------------------------
            # Run the pipeline.
            # ------------------------------------------------------------------
            if "base_path" in data_batch:
                base_path = data_batch["base_path"][0]
            else:
                base_path = None
            if base_path is not None:
                neg_t5_text_embeddings = torch.load(os.path.join(base_path, "kubric_generated/t5_xxl/common_neg_prompt-v1.pt"))
            else:
                neg_t5_text_embeddings = torch.load("/net/acadia1a/data/sriram/vidgen/datasets/kubric_generated/t5_xxl/common_neg_prompt-v1.pt")
            neg_t5_text_embeddings = neg_t5_text_embeddings.to(dtype=self.precision).contiguous()
            ...
            gen_video = self.pipe(
                ...
                prompt="",  # no text prompt provided during validation
                t5_text_embeddings=data_batch["t5_text_embeddings"] if "t5_text_embeddings" in data_batch else None,
            ...
            generated_videos.append(gen_video.cpu())

        # Dummy loss – model is not optimised during validation.
        loss = torch.zeros(1, device=video.device)
        output_batch = {"generated_videos": generated_videos}

        # Clear the CUDA memory for unwanted things in the validation step
