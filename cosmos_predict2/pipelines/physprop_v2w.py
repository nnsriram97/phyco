...
class PhyspropConditionedVideo2WorldPipeline(Video2WorldPipeline):
    ...
    def from_config(
        ...
        # Create a pipe
        pipe = PhyspropConditionedVideo2WorldPipeline(device=device, torch_dtype=torch_dtype)
        pipe.config = config
        pipe.precision = {
        ...
        if text_encoder_path:
            ...
            pipe.text_encoder = CosmosT5TextEncoder(device=device, cache_dir=text_encoder_path, use_8bit=True, local_files_only=True)
            pipe.text_encoder.to(device=device, dtype=torch_dtype)
            print(f"Successfully loaded text encoder from {text_encoder_path}")
        else:
            # training
        ...
        # Pass the existing tokenizer to the multi-branch ControlNet DiT.
        target = dit_config.get("_target_", "")
        target_name = getattr(target, '__name__', str(target))
        log.info(f"target_name: {target_name}")
        if "PhyspropControlNetDiTMultiple" == target_name:
            dit_config["physprop_tokenizer"] = pipe.tokenizer

        pipe.dit = instantiate(dit_config).eval()  # inference

        # # Print controlnet_output_projections weights before loading pre-trained model
        # if hasattr(pipe.dit, 'controlnet_output_projections'):
        #     print("\n=== ControlNet Output Projections Weights BEFORE Loading Pre-trained Model ===")
        #     for i, proj in enumerate(pipe.dit.controlnet_output_projections):
        #         print(f"Projection {i}:")
        #         print(f"  Weight shape: {proj.conv.weight.shape}")
        #         print(f"  Weight mean: {proj.conv.weight.mean().item():.6f}")
        #         print(f"  Weight std: {proj.conv.weight.std().item():.6f}")
        #         print(f"  Weight min: {proj.conv.weight.min().item():.6f}")
        #         print(f"  Weight max: {proj.conv.weight.max().item():.6f}")
        #         print(f"  Bias shape: {proj.conv.bias.shape}")
        #         print(f"  Bias mean: {proj.conv.bias.mean().item():.6f}")
        #         print(f"  Bias std: {proj.conv.bias.std().item():.6f}")
        #         print()
        ...
        if branch_ckpts:
            if hasattr(pipe.dit, "load_controlnet_branch_checkpoints"):
                ...
                pipe.dit.load_controlnet_branch_checkpoints(branch_ckpts)
            else:
        ...
        # # Print controlnet_output_projections weights after loading pre-trained model
        # if hasattr(pipe.dit, 'controlnet_output_projections'):
        #     print("\n=== ControlNet Output Projections Weights AFTER Loading Pre-trained Model ===")
        #     for i, proj in enumerate(pipe.dit.controlnet_output_projections):
        #         print(f"Projection {i}:")
        #         print(f"  Weight shape: {proj.conv.weight.shape}")
        #         print(f"  Weight mean: {proj.conv.weight.mean().item():.6f}")
    ...
    def _get_data_batch_input(
        self,
        video: torch.Tensor,
        physprop: torch.Tensor,
        prompt: str,
        negative_prompt: str = "",
        num_latent_conditional_frames: int = 1,
        t5_text_embeddings: torch.Tensor = None,
        neg_t5_text_embeddings: torch.Tensor = None,
        first_depth_frame: torch.Tensor = None,
        neg_physprop: torch.Tensor = None,
    ):
        """
        ...
        Args:
            ...
            physprop (torch.Tensor): The input physprop tensor (B, C, H, W).
            neg_physprop (torch.Tensor): The input neg_physprop tensor (B, C, H, W).
        ...
        if t5_text_embeddings is not None:
            t5_text_embeddings = t5_text_embeddings.to(dtype=self.torch_dtype).contiguous()
        elif len(prompt) > 0:
            t5_text_embeddings = self.encode_prompt(prompt).to(dtype=self.torch_dtype).contiguous()
        else:
            t5_text_embeddings = torch.zeros(1, 512, 1024, dtype=self.torch_dtype).contiguous()
        ...
        data_batch = {
            ...
            "physprop": physprop,
            "first_depth_frame": first_depth_frame,
        }

        # Handle negative prompts for classifier-free guidance
        if neg_t5_text_embeddings is not None:
            neg_t5_text_embeddings = neg_t5_text_embeddings.to(dtype=self.torch_dtype).contiguous()
            data_batch["neg_t5_text_embeddings"] = neg_t5_text_embeddings
        elif len(negative_prompt) > 0:
            neg_t5_text_embeddings = self.encode_prompt(negative_prompt).to(dtype=self.torch_dtype).contiguous()
            data_batch["neg_t5_text_embeddings"] = neg_t5_text_embeddings
        if neg_physprop is not None:
            neg_physprop = neg_physprop.to(dtype=self.torch_dtype).contiguous()
            data_batch["neg_physprop"] = neg_physprop

        # Move tensors to GPU and convert to bfloat16 if they are floating point
        for k, v in data_batch.items():
            if isinstance(v, torch.Tensor) and torch.is_floating_point(data_batch[k]):
                data_batch[k] = v.cuda().to(dtype=torch.bfloat16).contiguous()

        return data_batch

    def set_controlnet_mode(
        self,
        active: Union[None, str, int, Sequence[Union[str, int]]] = None,
        trainable: Union[None, str, int, Sequence[Union[str, int]]] = None,
        scales: Union[None, float, Sequence[float], Dict[Union[str, int], float]] = None,
    ) -> None:
        """Convenience wrapper for configuring ControlNet branches on the underlying DiT."""
        if not hasattr(self.dit, "set_active_controlnets"):
            raise AttributeError("Current DiT model does not expose ControlNet configuration methods.")

        if active is not None:
            self.dit.set_active_controlnets(active)
        if trainable is not None:
            self.dit.set_trainable_controlnets(trainable)
        if scales is not None and hasattr(self.dit, "set_controlnet_conditioning_scales"):
            self.dit.set_controlnet_conditioning_scales(scales)

    def load_controlnet_branch_weights(
        self,
        branch_to_checkpoint: Mapping[Union[str, int], str],
        strict: bool = True,
        map_location: Union[str, torch.device] = "cpu",
    ) -> Dict[Union[str, int], Any]:
        """Load ControlNet-specific checkpoints without disturbing the base model."""
        if not hasattr(self.dit, "load_controlnet_branch_checkpoints"):
            raise AttributeError("Current DiT model does not support branch-wise checkpoint loading.")
        return self.dit.load_controlnet_branch_checkpoints(branch_to_checkpoint, strict=strict, map_location=map_location)
    ...
    def __call__(
        ...
        seed: int = 0,
        first_depth_frame: np.ndarray = None,
        neg_physprop: np.ndarray = None,
    ) -> torch.Tensor | None:
        # Parameter check
        ...
        vid_input = torch.from_numpy(first_frame).permute(2, 0, 1)[None, :, None, ...].contiguous()
        if physprop.ndim == 3:
            physprop_input = torch.from_numpy(physprop).permute(2, 0, 1)[None, :, ...].contiguous()
        elif physprop.ndim == 1:
            physprop_input = torch.from_numpy(physprop).unsqueeze(0).contiguous()
        
        if neg_physprop is not None and neg_physprop.ndim == 3:
            neg_physprop_input = torch.from_numpy(neg_physprop).permute(2, 0, 1)[None, :, ...].contiguous()
        elif neg_physprop is not None and neg_physprop.ndim == 1:
            neg_physprop_input = torch.from_numpy(neg_physprop).unsqueeze(0).contiguous()
        else:
            neg_physprop_input = None

        first_depth_frame_input = None
        if first_depth_frame is not None:
            first_depth_frame_input = torch.from_numpy(first_depth_frame).permute(2, 0, 1)[None, :, ...].contiguous()
        ...
        data_batch = self._get_data_batch_input(
            ...
            first_depth_frame=first_depth_frame_input,
            neg_physprop=neg_physprop_input,
        )
        ...
        # Decode
        video = self.decode(samples).contiguous()  # shape: (B, C, T, H, W), possibly out of [-1, 1]
        ...
        if self.video_guardrail_runner is not None:
            ...
            frames = frames.permute(1, 2, 3, 0).contiguous().cpu().numpy()  # (T, H, W, C)
            ...
            processed_video = torch.from_numpy(processed_frames).float().permute(3, 0, 1, 2).contiguous() / 255.0
            processed_video = processed_video * 2 - 1  # back to [-1, 1]
            processed_video = processed_video.unsqueeze(0)

            video = processed_video.to(video.device, dtype=video.dtype).contiguous()
