...
from cosmos_predict2.data.dataset_utils import detect_aspect_ratio
...
def read_and_process_video(
    ...
    start_idx = 0
    end_idx = start_idx + frames_to_extract
    extracted_frames = video_tensor[:, start_idx:end_idx, :, :]  # (C, frames_to_extract, H, W)
...
class Video2WorldPipeline(BasePipeline):
    ...
    def from_config(
        ...
        if text_encoder_path:
            ...
            pipe.text_encoder = CosmosT5TextEncoder(device=device, cache_dir=text_encoder_path, local_files_only=True, use_8bit=True)
    ...
    def decode_with_grad(self, latent: torch.Tensor, use_cp: bool = True) -> torch.Tensor:
        """Decode latent with gradient preservation.

        Args:
            latent: Latent tensor to decode [B, C, T, H, W]
            use_cp: Whether to use context parallelism if available (default: True)

        Returns:
            Decoded video tensor with gradients preserved
        """
        cp_group = None
        if use_cp:
            cp_group = self.get_context_parallel_group()
            # Only use CP if group exists and has multiple ranks
            if cp_group is not None and cp_group.size() <= 1:
                cp_group = None

        return self.tokenizer.decode_with_grad(latent / self.sigma_data, cp_group=cp_group)
    ...
    def broadcast_split_for_model_parallelsim(
        ...
        use_context_parallel = condition.is_video and cp_size > 1

        if use_context_parallel and x0_B_C_T_H_W.shape[2] % cp_size != 0:
            log.warning(
                f"Context parallel fallback: sequence length {x0_B_C_T_H_W.shape[2]} is not divisible by cp_size {cp_size}; running this batch without context parallelism."
            )
        ...
        if use_context_parallel:
            ...
            if condition is not None:
                condition = condition.broadcast(cp_group, allow_split=True)
            ...
            if cp_group is not None:
                ...
                if sigma_B_T is not None:
                    sigma_B_T = broadcast(sigma_B_T, cp_group)
                if condition is not None:
                    condition = condition.broadcast(cp_group, allow_split=False)
    ...
    def get_x0_fn_from_batch(
        ...
        dit_override: Optional[nn.Module] = None,
    ) -> Callable:
        """
        ...
        dit_module: nn.Module = dit_override or self.dit
        restore_dit: Optional[nn.Module] = None
        if dit_override is not None and self.dit is not dit_module:
            restore_dit = self.dit
            self.dit = dit_module
        ...
        def _denoise_with_module(
            module: nn.Module, noise_x: torch.Tensor, sigma: torch.Tensor, cond: T2VCondition
        ) -> torch.Tensor:
            if module is self.dit:
                return self.denoise(noise_x, sigma, cond).x0
            prev_dit = self.dit
            self.dit = module
            try:
                return self.denoise(noise_x, sigma, cond).x0
        ...
        def x0_fn(noise_x: torch.Tensor, sigma: torch.Tensor) -> torch.Tensor:
            cond_x0 = _denoise_with_module(dit_module, noise_x, sigma, condition)
            uncond_x0 = _denoise_with_module(dit_module, noise_x, sigma, uncondition)
        ...
        if first_frame is not None:
            ...
            if ext in _VIDEO_EXTENSIONS:
                ...
                if num_latent_conditional_frames == 1:
                    ...
                    # Read the image and get the aspect ratio
                    img = Image.open(input_path)
                    aspect_ratio = detect_aspect_ratio((img.width, img.height))  # expects (W, H)
                    log.info(f"aspect_ratio: {aspect_ratio}")
                    height, width = VIDEO_RES_SIZE_INFO[self.config.resolution][aspect_ratio]
                    log.info(f"width: {width}, height: {height}")
                    vid_input = read_and_process_image(input_path, [width, height], num_video_frames, resize=True)
                else:
                    raise ValueError(
