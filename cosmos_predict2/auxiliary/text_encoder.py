...
class CosmosT5TextEncoder(torch.nn.Module):
        ...
        try:
            if use_8bit:
                ...
                self.tokenizer = T5TokenizerFast.from_pretrained(
                    pretrained_model_name_or_path=cache_dir, cache_dir=cache_dir, local_files_only=local_files_only, low_cpu_mem_usage=True, quantization_config=bnb_config
                )
                ...
                self.tokenizer = T5TokenizerFast.from_pretrained(
                    pretrained_model_name_or_path=cache_dir, cache_dir=cache_dir, local_files_only=local_files_only, low_cpu_mem_usage=True
                )
                self.text_encoder = T5EncoderModel.from_pretrained(
                    pretrained_model_name_or_path=cache_dir, cache_dir=cache_dir, local_files_only=local_files_only, low_cpu_mem_usage=True
                ).to(device)
            ...
            log.error(e)
            self.tokenizer = T5TokenizerFast.from_pretrained(
                pretrained_model_name_or_path=model_name, cache_dir=cache_dir, local_files_only=local_files_only, low_cpu_mem_usage=True
            )
            self.text_encoder = T5EncoderModel.from_pretrained(
                pretrained_model_name_or_path=model_name, cache_dir=cache_dir, local_files_only=local_files_only, low_cpu_mem_usage=True
            ).to(device)
    ...
    def encode_prompts(
        ...
        # Ensure the output is in float32 precision to avoid issues with fused operations
        encoded_text = encoded_text.float()
