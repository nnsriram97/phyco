...
class Checkpointer:
    ...
    def save(
        ...
        # Handle model wrapped by DeepSpeed or other distributed frameworks
        if hasattr(model, 'module'):
            # Model is wrapped (e.g., by DeepSpeed, DDP, etc.)
            original_model = model.module
        else:
            original_model = model
        
        is_fsdp = original_model.config.fsdp_shard_size != 0 and distributed.get_world_size() > 1
        if is_fsdp:
            ...
            if is_fsdp:
                ...
                set_model_state_dict(
                    ...
                    options=StateDictOptions(
                        ...
                        strict=False if original_model.config.train_architecture == "lora" else True,
                    ),
                )
                # Load EMA weights.
                if original_model.pipe.config.ema.enabled:
                    set_model_state_dict(
                        model.pipe.dit_ema,
