...
class Vid2VidCondition(T2VCondition):
    ...
    def broadcast(
        ...
        should_split = (
            ...
            and process_group.size() > 1
            and gt_frames is not None
            and T is not None
            and T > 1
            and T % process_group.size() == 0
        )
        ...
        if should_split:
            ...
            gt_frames = broadcast(gt_frames, process_group)
            condition_video_input_mask_B_C_T_H_W = broadcast(
                condition_video_input_mask_B_C_T_H_W, process_group
            )
